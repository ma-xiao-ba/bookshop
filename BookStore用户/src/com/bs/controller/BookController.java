...
public class BookController {
	...
	public String queryHottestBooks(HttpSession session){
		...
		List<BookTypes> bookTypes=bookTypeService.quaryBookType();
		for(int i=0;i<bookTypes.size();i++){
		
			System.out.println(bookTypes.get(i).getBtname()+" "+bookTypes.get(i).getBtid());
		}
		session.setAttribute("queryBookType", bookTypes);
		return "index";
	}
