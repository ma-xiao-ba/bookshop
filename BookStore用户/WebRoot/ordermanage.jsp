						...
						<table id="ordermanage" class="timetable_sub">
							...
							<form action="modifyOrder.action" method="post">
								<tr>
										...
										<td>
											<a href="queryOrderByIsbn.action?oisbn=${ods.oisbn}">详情</a>
										</td>
										<td>
											<div>
											<c:if test="${ods.ostate==0}">
													<a href="javascript:return false;" class="btn btn-warning" style="background-color:#C0C0C0">未发货</a>
												</c:if>
												<c:if test="${ods.ostate==1}">
													<a href="changeState.action?ostate=2&oid=${ods.oid}&uname=zhangsan" class="btn btn-warning">确认收货</a>
												</c:if>
												<c:if test="${ods.ostate==2}">
